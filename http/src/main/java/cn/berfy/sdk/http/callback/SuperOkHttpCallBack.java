...
public class SuperOkHttpCallBack<T> {
    ...
    RequestCallBackH5<T> mRequestCallBackH5;
            ...
            try {
                ...
                try {
                    if (!TextUtils.isEmpty(mRequestCode)) {
                        ...
                        netResponse.data = getContent(response);
                    } else {
...
//                            LogF.d(Constant.HTTPTAG, "类型  " + type.getClass().getSimpleName());
                                netResponse.data = (T) GsonUtil.getInstance().toClass(data, type);
                            } catch (Exception e) {
//                            LogF.d(Constant.HTTPTAG, "类型有问题");
...
//                        LogF.d(Constant.HTTPTAG, "类型  null != t");
                    ...
                    e.printStackTrace();
                    LogF.d(Constant.HTTPTAG, "data数据解析失败：\n" + e.getMessage() + "\n" + e.getCause());
                }
//                netResponse.netMessage = netMessage;
                ...
                e.printStackTrace();
                LogF.d(Constant.HTTPTAG, "数据解析失败：\n" + e.getMessage() + "\n" + e.getCause());
            }
        }
    }
